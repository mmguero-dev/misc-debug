...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.logo_label = customtkinter.CTkLabel(
            self.left_side_panel, text="Malcolm Configuration \n", font=customtkinter.CTkFont(size=20, weight="bold")
        )
        ...
        self.scaling_optionemenu = customtkinter.CTkOptionMenu(
            self.left_side_panel, values=["80%", "90%", "100%", "110%", "120%"], command=self.change_scaling_event
        )
        self.scaling_optionemenu.grid(row=8, column=0, padx=20, pady=(10, 20), sticky="s")

        self.bt_Quit = customtkinter.CTkButton(
            self.left_side_panel, text="Quit", fg_color='#EA0000', hover_color='#B20000', command=self.close_window
        )
        ...
        self.bt_categories = customtkinter.CTkButton(
            self.left_side_panel, text="Manage Categories", command=self.categories
        )
        ...
        self.right_dashboard.pack(
            in_=self.right_side_panel, side=tkinter.TOP, fill=tkinter.BOTH, expand=True, padx=0, pady=0
        )

    #  self.right_dashboard   ----> dashboard widget
    def dash(self):
        self.clear_frame()
        self.bt_from_frame1 = customtkinter.CTkButton(
            self.right_dashboard, text="dash", command=lambda: print("test dash")
        )
        self.bt_from_frame1.grid(row=0, column=0, padx=20, pady=(10, 0))
        self.bt_from_frame2 = customtkinter.CTkButton(
            self.right_dashboard, text="dash 1", command=lambda: print("test dash 1")
        )
    ...
    def statement(self):
        self.clear_frame()
        self.bt_from_frame3 = customtkinter.CTkButton(
            self.right_dashboard, text="statement", command=lambda: print("test statement")
        )
    ...
    def categories(self):
        self.clear_frame()
        self.bt_from_frame4 = customtkinter.CTkButton(
            self.right_dashboard, text="categories", command=lambda: print("test cats")
        )
